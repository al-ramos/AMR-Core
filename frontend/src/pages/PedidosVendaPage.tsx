...
export default function PedidosVendaPage() {
  ...
  // Busca clientes sempre — para nome real na tabela E para modal
  ...
  const criar = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['pedidos-venda'] }); fecharModal() },
  ...
  function fecharModal() {
    setModalNovo(false); setClienteId(''); setObservacao('')
            ...
            <div className="table-responsive">
              <table className="table table-hover table-sm mb-0" style={{ fontSize: 13 }}>
                <thead className="table-light">
                  <tr>
                    <th>#</th><th>Cliente</th><th>Emissão</th><th>Aprovação</th>
                    <th>Faturamento</th><th>Status</th>
                ...
                <tbody>
                  {paged.map(p => (
                    <tr key={p.id}>
                      ...
                      <td className="fw-medium">{clienteMap[p.clienteId] ?? `Cliente ${p.clienteId}`}</td>
